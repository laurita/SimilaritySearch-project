...
An object $A$ is only matched to object $B$ if the object $A$ is the unique, most similar object to $B$ and vice versa. In other words the matrix entry for $AB$ is the unique, smallest entry in this row and column.
...
This algorithm matches as many items as possible, i.e min(\#rows, \#columns). Therefore, it would match even very different items, if there are no better matches left. The algorithm yields a stable matching, which is proven in \cite{augsten}.
...
The latter case is readily applicable for the item matching problem. The difference is that for matching items the input is a distance matrix. Therefore, to use the algorithm the row-wise and column-wise rankings of the distances have to be calculated (the smaller the distance, the better the ranking).
...
\subsection{Proof: Global Greedy Equals Stable Marriage\label{proofggsm}}
...
For $n = 1$ this is clear, since only one match is possible.
...
This means that this match will be included in any SM and GG matching
...
The comparison of the algorithms includes runtime and quality tests. In our runtime tests we use randomly generated distance matrices of increasing size. In the quality tests, we use the Bolzano Address Tree distance data and compute recall and precision for all four algorithms.
...
We have shown that GG and SM will produce equal results if the matrix entries are unique, but we did not check whether the matchings were identical in our case. Since most non-unique values in the data files are ones, the algorithms might not "reach" them and this could well be the case.
...
The runtime check lets you test the runtime of a selection (or all) of the discussed algorithms. You can select the starting size $n$ of a $n$x$n$ matrix, the step-size by which to increase this $n$ and the amount of steps to perform. Be careful not to choose the amount of steps too high, as this will result in memory exceptions and very long run times. The matrices are generated using random double values in [0,1].
