...
public class HungarianAlgorithm {
    ...
    public int[][] originalComputeAssignments(double[][] matrixInp) {
    	...
    	double[][] matrix = MatrixTools.cloneMatrix(matrixInp);
        ...
        for (int i = 0; i < matrix[0].length; i++) {
            if (starsByCol[i] != -1) {
            	if (transposed) {
            		retval[i] = new int[]{i, starsByCol[i]};
            	} else {
     ...
     */
    private void reduceMatrixAndInitialize(double[][] matrix) {
        ...
        for (int row = 0; row < matrix.length; row++) {
            ...
            if (minValInRow < Double.MAX_VALUE) {
                for (int col = 0; col < matrix[row].length; col++) {
                    if (matrix[row][col] < Double.MAX_VALUE) {
    ...
    private void initStars(double costMatrix[][]) {
     ...
     */
    private boolean makeMoreZeroes(double[][] matrix) {
